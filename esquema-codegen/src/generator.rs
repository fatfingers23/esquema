...
pub(crate) fn generate_schemas(
    ...
    if let Some(basename) = paths.pop() {
        ...
        for (name, def) in &schema.defs {
            ...
            // main def
            if name == "main" {
                tokens.push(user_type(def, &schema.id, basename, true, module_name)?);
            } else {
                names.push(name);
            }
        }
        ...
        // ref unions
        tokens.push(ref_unions(
            &schema.id,
            &find_ref_unions(&schema.defs),
            module_name,
        )?);
...
pub(crate) fn generate_client(
    ...
    let tokens = client(&tree, &schema_map, namespaces, module_name)?;
    let content = quote! {
